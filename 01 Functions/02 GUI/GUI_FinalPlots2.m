...
% Last Modified by GUIDE v2.5 24-May-2018 00:42:21
...
set(handles.PlotSa_ad1,'XData',x(value, value:end),'YData',Sa_nom_ad1(value, value:end,1));
...
handles.PlotSa_ad1   = plot(x(1,:), Sa_nom_ad1(end,:),'b');
handles.PlotStot_ad1 = plot(x(1,:), St_nom_ad1(end,:),'g');
handles.PlotSy       = plot(x(1,:), Sy*ones(1,size(x,2)),'--c');
hold off
title('Load Cycle Stress')
legend('S_{m,nom}', 'S_{a}' ,'S_{tot}','Yield Stress \sigma_{y}')
ylabel('Stress [MPa]')
xlabel('Distance x [mm]')
xlim([x(1,1) x(1,end)]);
