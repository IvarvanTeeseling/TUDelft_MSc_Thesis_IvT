function [Shr_a, Pl_a, Ad_1, Ad_2] = Adhesive_Stresses_LuoTong(x, F_k, M_k, Q_k, D_1, A_11, l_B, E, t, E_a, G_a, t_a)
%% Description
% Syntax
...
% Important note: applying the k factor for M_k and Q_k from GR
...
% Peel stress integration constants (From Luo and Tong 2004)
...
Bnd_l   = -E_a.*(-(B_s1+B_s4).*cosh(beta_s.*-l_B).*sin(beta_s.*-l_B)+sinh(beta_s.*-l_B).*cos(beta_s.*-l_B).*(B_s1-B_s4))./(t_a.*beta_s);
Bnd_u   = -E_a.*(-(B_s1+B_s4).*cosh(beta_s.*x   ).*sin(beta_s.*x   )+sinh(beta_s.*x   ).*cos(beta_s.*x   ).*(B_s1-B_s4))./(t_a.*beta_s);
Q_1     = Q_k-(Bnd_u-Bnd_l);
...
C1 = Q_k+(-E_a.*(-(B_s1+B_s4).*cosh(beta_s.*-l_B).*sin(beta_s.*-l_B)+sinh(beta_s.*-l_B).*cos(beta_s.*-l_B).*(B_s1-B_s4))./(t_a.*beta_s));
C2  = (-E_a*(B_s1.*exp(beta_s.*-l_B).*cos(beta_s.*-l_B)+B_s1.*exp(-beta_s.*-l_B).*cos(beta_s.*-l_B)-B_s4.*exp(beta_s.*-l_B).*sin(beta_s.*-l_B)+B_s4.*exp(-beta_s.*-l_B).*sin(beta_s.*-l_B))./(2*t_a*beta_s.^2));
C3  = (-E_a*(B_s1.*exp(beta_s.*x).*cos(beta_s.*x)+B_s1.*exp(-beta_s.*x).*cos(beta_s.*x)-B_s4.*exp(beta_s.*x).*sin(beta_s.*x)+B_s4.*exp(-beta_s.*x).*sin(beta_s.*x))./(2*t_a*beta_s.^2));
C4  = -C3+C2+C1.*(l_B+x);
S1  = (F_k*t+6*M_k).*sinh(beta_t*-l_B)./(8*t*sinh(beta_t*l_B))+3*(F_k*t-2*M_k)./(8*t*l_B).*-l_B;
S2  = (F_k*t+6*M_k).*sinh(beta_t*x)./(8*t*sinh(beta_t*l_B))+3*(F_k*t-2*M_k)./(8*t*l_B).*x;
M_1 = M_k+C4-(t+t_a)/2*(S2-S1);
...
C1 = (-E_a*(B_s1.*exp(beta_s.*x).*cos(beta_s.*x)+B_s1.*exp(-beta_s.*x).*cos(beta_s.*x)-B_s4.*exp(beta_s.*x).*sin(beta_s.*x)+B_s4.*exp(-beta_s.*x).*sin(beta_s.*x))./(2*t_a*beta_s.^2));
C2 = (-E_a*(B_s1.*exp(beta_s.*-l_B).*cos(beta_s.*-l_B)+B_s1.*exp(-beta_s.*-l_B).*cos(beta_s.*-l_B)-B_s4.*exp(beta_s.*-l_B).*sin(beta_s.*-l_B)+B_s4.*exp(-beta_s.*-l_B).*sin(beta_s.*-l_B))./(2*t_a*beta_s.^2));
C3 = (-E_a.*(-(B_s1+B_s4).*cosh(beta_s.*-l_B).*sin(beta_s.*-l_B)+sinh(beta_s.*-l_B).*cos(beta_s.*-l_B).*(B_s1-B_s4))./(t_a.*beta_s));
C4 = C1-C2-C3.*(x+l_B);
M_2 = C4-(t+t_a)/2*(S2-S1);
